import AnimatedElement from "./animated-section";

export default function SectionExperience() {
  return (
    <div className="wrapper flex flex-col gap-10 lg:gap-16 w-full">
      <AnimatedElement>
        <h2 className="font-heading font-semibold text-5xl lg:text-7xl">
          experience
        </h2>
      </AnimatedElement>
      {/* Experience 1 */}
      <AnimatedElement delay={0.1}>
        <div className="flex self-stretch justify-start items-start flex-col lg:flex-row">
        <div className="w-full lg:w-1/3 flex border-solid border-[#2B2B2B] border-b py-3">
          <p className="flex-1 text-accent-light text-sm font-['Kode_Mono']">
            August 2022 – July 2025
          </p>
        </div>
        <div className="flex flex-1 w-full justify-start items-start flex-col gap-2.5">
          <div className="flex self-stretch justify-start items-start flex-col">
            <div className="flex self-stretch justify-start items-start flex-row border-solid border-[#2B2B2B] border-b">
              <div className="flex flex-1 justify-center items-center flex-row gap-2.5 py-3">
                <p className="flex-1 text-link-green text-sm font-['Kode_Mono']">
                  Front-End Developer | Team Lead
                </p>
              </div>
              <div className="flex flex-1 justify-start items-center flex-row gap-2.5 py-3">
                <p className="flex-1 text-accent text-sm font-['Kode_Mono'] text-right">
                  Theory Digital, Edmonton, AB
                </p>
              </div>
            </div>
            <div className="flex self-stretch justify-start items-start flex-row">
              <div className="flex flex-1 justify-center items-center flex-row gap-2.5 py-3 border-solid border-[#2B2B2B] border-b">
                <p className="flex-1 text-sm font-['Kode_Mono']">
                  Architected and developed custom Shopify themes and advanced features using Liquid templating, modern CSS, and JavaScript Web Components with accessibility and performance optimization top of mind
                </p>
              </div>
            </div>
            <div className="flex self-stretch justify-start items-start flex-row">
              <div className="flex flex-1 justify-center items-center flex-row gap-2.5 py-3 border-solid border-[#2B2B2B] border-b">
                <p className="flex-1 text-sm font-['Kode_Mono']">
                  Engineered scalable client websites and interactive features leveraging Next.js with Prismic Headless CMS integration
                </p>
              </div>
            </div>
            <div className="flex self-stretch justify-start items-start flex-row">
              <div className="flex flex-1 justify-center items-center flex-row gap-2.5 py-3 border-solid border-[#2B2B2B] border-b">
                <p className="flex-1 text-sm font-['Kode_Mono']">
                  Optimized and enhanced client web applications using Statamic CMS, implementing custom functionality and performance improvements
                </p>
              </div>
            </div>
            <div className="flex self-stretch justify-start items-start flex-row">
              <div className="flex flex-1 justify-center items-center flex-row gap-2.5 py-3 border-solid border-[#2B2B2B] border-b">
                <p className="flex-1 text-sm font-['Kode_Mono']">
                  Designed compelling user interfaces and interactive prototypes using Figma, translating client visions into pixel-perfect digital experiences
                </p>
              </div>
            </div>
            <div className="flex self-stretch justify-start items-start flex-row">
              <div className="flex flex-1 justify-center items-center flex-row gap-2.5 py-3 border-solid border-[#2B2B2B] border-b">
                <p className="flex-1 text-sm font-['Kode_Mono']">
                  Led cross-functional collaboration with clients and stakeholders, managing project timelines and delivering high-quality solutions on schedule
                </p>
              </div>
            </div>
            <div className="flex self-stretch justify-start items-start flex-row">
              <div className="flex flex-1 justify-center items-center flex-row gap-2.5 py-3 border-solid border-[#2B2B2B] border-b">
                <p className="flex-1 text-sm font-['Kode_Mono']">
                  Facilitated team stand-ups, managed task distribution and deadlines, and provided technical mentorship to junior developers
                </p>
              </div>
            </div>
          </div>
        </div>
      </div>
      </AnimatedElement>
      {/* Experience 2 */}
      <AnimatedElement delay={0.15}>
        <div className="flex self-stretch justify-start items-start flex-col lg:flex-row">
        <div className="w-full lg:w-1/3 flex border-solid border-[#2B2B2B] border-b py-3">
          <p className="flex-1 text-accent-light text-sm font-['Kode_Mono']">
            October 2021 - July 2022
          </p>
        </div>
        <div className="flex flex-1 w-full justify-start items-start flex-col gap-2.5">
          <div className="flex self-stretch justify-start items-start flex-col">
            <div className="flex self-stretch justify-start items-start flex-row border-solid border-[#2B2B2B] border-b">
              <div className="flex flex-1 justify-center items-center flex-row gap-2.5 py-3">
                <p className="flex-1 text-link-green text-sm font-['Kode_Mono']">
                  UI UX Developer
                </p>
              </div>
              <div className="flex flex-1 justify-start items-center flex-row gap-2.5 py-3">
                <p className="flex-1 text-accent text-sm font-['Kode_Mono'] text-right">
                  Riva International, Edmonton, AB
                </p>
              </div>
            </div>
            <div className="flex self-stretch justify-start items-start flex-row">
              <div className="flex flex-1 justify-center items-center flex-row gap-2.5 py-3 border-solid border-[#2B2B2B] border-b">
                <p className="flex-1 text-sm font-['Kode_Mono']">
                  Built responsive and interactive front-end interfaces for enterprise applications using ASP.NET Core and Vue.js frameworks
                </p>
              </div>
            </div>
            <div className="flex self-stretch justify-start items-start flex-row">
              <div className="flex flex-1 justify-center items-center flex-row gap-2.5 py-3 border-solid border-[#2B2B2B] border-b">
                <p className="flex-1 text-sm font-['Kode_Mono']">
                  Produced comprehensive design systems, interactive prototypes, and detailed wireframes in Figma to drive feature development initiatives
                </p>
              </div>
            </div>
            <div className="flex self-stretch justify-start items-start flex-row">
              <div className="flex flex-1 justify-center items-center flex-row gap-2.5 py-3 border-solid border-[#2B2B2B] border-b">
                <p className="flex-1 text-sm font-['Kode_Mono']">
                  Conducted user research and stakeholder interviews to identify pain points and implement data-driven UX improvements
                </p>
              </div>
            </div>
          </div>
        </div>
      </div>
      </AnimatedElement>
      {/* Experience 3 */}
      <AnimatedElement delay={0.2}>
        <div className="flex self-stretch justify-start items-start flex-col lg:flex-row">
        <div className="w-full lg:w-1/3 flex border-solid border-[#2B2B2B] border-b py-3">
          <p className="flex-1 text-accent-light text-sm font-['Kode_Mono']">
            January, 2021 - October, 2021
          </p>
        </div>
        <div className="flex flex-1 w-full justify-start items-start flex-col gap-2.5">
          <div className="flex self-stretch justify-start items-start flex-col">
            <div className="flex self-stretch justify-start items-start flex-row border-solid border-[#2B2B2B] border-b">
              <div className="flex flex-1 justify-center items-center flex-row gap-2.5 py-3">
                <p className="flex-1 text-link-green text-sm font-['Kode_Mono']">
                  Front-End Developer & Designer
                </p>
              </div>
              <div className="flex flex-1 justify-start items-center flex-row gap-2.5 py-3">
                <p className="flex-1 text-accent text-sm font-['Kode_Mono'] text-right">
                  Rent Panda, Thunder Bay, ON
                </p>
              </div>
            </div>
            <div className="flex self-stretch justify-start items-start flex-row">
              <div className="flex flex-1 justify-center items-center flex-row gap-2.5 py-3 border-solid border-[#2B2B2B] border-b">
                <p className="flex-1 text-sm font-['Kode_Mono']">
                  Developed and implemented custom front-end features and platform enhancements for the Rent Panda ecosystem using React.js
                </p>
              </div>
            </div>
            <div className="flex self-stretch justify-start items-start flex-row">
              <div className="flex flex-1 justify-center items-center flex-row gap-2.5 py-3 border-solid border-[#2B2B2B] border-b">
                <p className="flex-1 text-sm font-['Kode_Mono']">
                  Created user-centered design mockups, interactive prototypes, and comprehensive wireframes in Figma to guide feature development
                </p>
              </div>
            </div>
          </div>
        </div>
      </div>
      </AnimatedElement>
      {/* Experience 4 */}
      <AnimatedElement delay={0.25}>
        <div className="flex self-stretch justify-start items-start flex-col lg:flex-row">
        <div className="w-full lg:w-1/3 flex border-solid border-[#2B2B2B] border-b py-3">
          <p className="flex-1 text-accent-light text-sm font-['Kode_Mono']">
            March, 2019 - August, 2022
          </p>
        </div>
        <div className="flex flex-1 w-full justify-start items-start flex-col gap-2.5">
          <div className="flex self-stretch justify-start items-start flex-col">
            <div className="flex self-stretch justify-start items-start flex-row border-solid border-[#2B2B2B] border-b">
              <div className="flex flex-1 justify-center items-center flex-row gap-2.5 py-3">
                <p className="flex-1 text-link-green text-sm font-['Kode_Mono']">
                  Web Developer & Designer | Part Time
                </p>
              </div>
              <div className="flex flex-1 justify-start items-center flex-row gap-2.5 py-3">
                <p className="flex-1 text-accent text-sm font-['Kode_Mono'] text-right">
                  Innovated Solutions, Thunder Bay, ON
                </p>
              </div>
            </div>
            <div className="flex self-stretch justify-start items-start flex-row">
              <div className="flex flex-1 justify-center items-center flex-row gap-2.5 py-3 border-solid border-[#2B2B2B] border-b">
                <p className="flex-1 text-sm font-['Kode_Mono']">
                  Designed, developed, and maintained custom WordPress websites for diverse client portfolios, ensuring optimal performance and user experience
                </p>
              </div>
            </div>
            <div className="flex self-stretch justify-start items-start flex-row">
              <div className="flex flex-1 justify-center items-center flex-row gap-2.5 py-3 border-solid border-[#2B2B2B] border-b">
                <p className="flex-1 text-sm font-['Kode_Mono']">
                  Produced engaging visual content including custom graphics, animations, and multimedia assets to support digital marketing campaigns
                </p>
              </div>
            </div>
          </div>
        </div>
      </div>
      </AnimatedElement>
    </div>
  );
}
